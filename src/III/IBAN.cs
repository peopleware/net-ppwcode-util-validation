...
namespace PPWCode.Util.Validation.III
{
    [Serializable]
    [DataContract]
    public class IBAN : AbstractIdentification
    {
        ...
        protected override bool OnValidate(string identification)
        {
            ...
            // Fase 2: Check the length of the IBAN nr according the country code
            if (ibanCountry.IBANLength != sb.Length)
            {
                return false;
            }

            // Fase 3: Check regular expression if known
            if (ibanCountry.Pattern != string.Empty)
            {
                Regex regex = new Regex(string.Concat("^", countryCode, @"\d{2}", ConvertToRegEx(ibanCountry.Pattern), "$"));
            ...
            // Fase 4: Move the first four characters of the IBAN to the right of the number.
